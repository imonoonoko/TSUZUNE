...
- When using TSUZUNE for a task, follow [the caller reading, verification, and completion procedure](docs/mcp-integration.md#作業の読取検証終了). Reuse sufficient source/revision evidence, map success conditions to what each check proves, and resume only unfinished work. This does not add mandatory retrieval or recording to No-TSUZUNE tasks.
