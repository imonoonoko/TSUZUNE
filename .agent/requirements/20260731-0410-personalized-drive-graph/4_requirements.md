...
Google Cloudから取得したDesktop OAuthクライアントJSONを選択し、`installed.client_id`を検証する。不正な設定を保存しない。
...
4. OAuth設定がない場合、Google機能だけが設定案内を表示し、ノート操作は正常に動く。
...
11. 実Google認証は本人のOAuthクライアントJSONを用いた手動確認項目として明示される。
