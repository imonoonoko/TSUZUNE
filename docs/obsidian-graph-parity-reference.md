...
- GP0-3b-nではattachment nodeの`デフォルトアプリで開く`を比較した。Obsidianはrendererの`window.open`へfile URLと`_external`、TSUZUNEは既存trusted IPCとVault validation後の`electron.shell.openPath`へabsolute filesystem pathを渡す。API表現は異なるが、同じ`attachments/diagram.svg`へのrequest 1回、menu close、Graph／Vault保持、同一process内のGraph再表示でのrequest非再生を満たしたため`matched-core-behavior`とする。builderは固定Obsidian raw observationのquery、camera、node ID集合、edge signature、Graph tab／leaf不変をmanifestと別に直接gateする。実OS既定appの選択・起動、chooser／cancelは未証明である。Obsidianは安全境界により別processを起動せず再起動時の非再生は未観測、TSUZUNEの再起動0回は追加証拠だけとする。context menu全体は11対8である。最新正本は[比較report](reports/graph-gp0-attachment-default-app-2026-08-10.html)と[機械可読comparison](reports/assets/graph-gp0-attachment-default-app/comparison.json)とする。
- GP0-3b-oではattachment nodeの`フォルダで表示`を比較した。Obsidianは`electron.remote.shell.showItemInFolder`、TSUZUNEは既存trusted IPCとVault validation後の`electron.shell.showItemInFolder`へ同じ`attachments/diagram.svg`の親フォルダ要求を1回渡す。両製品でmenu close、Graph／Vault保持、同一process内のGraph再表示でのrequest非再生を満たしたため`matched-core-behavior`とする。TSUZUNEはmain import前hookで別process再起動後0回も追加確認したが、Obsidianの別process再起動は安全境界により未観測である。Windows Explorerの実起動・表示結果、物理入力、実OSアクセシビリティ、pixel identityは未証明で、context menu全体は11対9である。最新正本は[比較report](reports/graph-gp0-attachment-folder-reveal-2026-08-11.html)と[機械可読comparison](reports/assets/graph-gp0-attachment-folder-reveal/comparison.json)とする。
- GP0-3b-pは`ファイルエクスプローラでファイルを表示`の固定参照checkpointである。fresh isolated captureで対象clickへ到達し、`attachments/diagram.svg`に対する`internalPlugins.file-explorer.revealInFolder`を1回確認した。menu close後に内部File Explorerが対象を選択し、external `window.open`／`shell.showItemInFolder`は0回である。full gateが不成立だった理由は、Graph再表示cameraがaction baselineと完全一致しなかった一点だけである。pは`blocked`として閉じ、再試行やcamera契約変更を自動では行わない。固定参照、hook／state observer、安全境界、停止条件の正本は[GP0-3b-p requirements](../.agent/requirements/20260811-0257-attachment-file-explorer-reveal/4_requirements.md)、診断結果は[diagnostic report](reports/graph-gp0-attachment-file-explorer-2026-08-11.md)と[comparison JSON](reports/assets/graph-gp0-attachment-file-explorer/comparison.json)とする。TSUZUNEの製品source、comparison parity claim、本番反映は未実施である。
...
3. GP0-3b-eのcontext menu項目比較からGP0-3b-oのattachment folder revealまでを、一項目ずつ固定比較した。GP0-3b-pは意味captureを完了したがcamera完全一致でblockedであり、camera契約の明示的な受入またはstop gate変更なしに再試行しない。残るmenu操作は、新しく選んだ独立sliceとしてだけ進め、pを再開する場合も同じ事前判断を満たす場合に限る。Excluded filesのManage UIと全機能共通効果、Searchのmalformed query境界も一項目ずつ比較する。
