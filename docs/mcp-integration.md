...
検索・参照と`autonomous_update_note`は自動利用でき、通常ノートのAI更新をユーザー承認なしで実行できます。`create_note`と通常の`update_note`は従来どおり確認を出す設定で登録します。
...
| `autonomous_update_note` | 承認を待たず通常ノートを更新し、旧本文・理由・出典を履歴へ保存 | 本文10万文字 |
...
- `autonomous_update_note`は通常ノートを自動更新し、更新前本文を`50_履歴/AI更新`へ保存します。`reason`、`source_refs`、`previous_revision`を出力し、原文・会話ログの自動更新には使いません。
