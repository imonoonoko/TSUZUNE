...
| 最優先Track | 製品Primary Trackは0。Workflow Verification Harness Phase 1、Executable Policy Pilot 1、MCP read-only完全化を完了。宣言済みread-only MCP 10件はcold／malformed／noncanonicalなcreation-time sidecarを含むVault／profile全体へ書き込まず、通常scanとwrite経路のrepairは維持する。次は自然利用観測 | [PLAN.md](PLAN.md#current-decision)／[MCP read-only完全化](docs/reports/mcp-readonly-zero-write-2026-08-26.md) |
