...
| Google OAuth reconnect correction | 失効済みrefresh tokenの再認証時、Googleのcanonical userinfo scopeを短縮scopeと同値判定できず拒否していた。`f6e85f4`で比較だけを正規化し、公式production update 10/10 checksをPASS。修正版での再認証／Drive read実runtime受入が残る | [report](docs/reports/google-oauth-scope-reconnect-2026-08-14.md) |
